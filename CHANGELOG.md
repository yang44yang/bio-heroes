...
## 钻石终于有用了：💎20 必出 SP 觉醒卡（2026-08-30）✅ `e9ab3b1`
> 起因是一句提问：「钻石有什么用？」查下来答案是 —— **一次都花不出去**。
> `addDiamonds` 有、`spendDiamonds` 没有、`canAfford` 只看金币；而首页和抽卡界面两处常驻
> 显示 💎N、闯关还专门弹窗奖励「+10💎」。通关全部 29 关攒 90 颗，一颗换不到东西。
> 这是「界面在说谎」里最直接的一种：数字在涨，但它什么都不是（比没有更难受）。

**用途选「必出 SP」不是拍脑袋**：三条路必须互不重复 —— 金币 → 普通池 · SSR 券 → 下次必出 SSR ·
钻石 → 必出 SP。而 `useGacha.js` 的注释白纸黑字记着「齐齐反馈"抽不到 SP"」，当时只给了 2%
基础概率（平均 50 抽才见一张）。钻石给他一条**确定的**路，SP 又是全游戏最有戏剧性的东西。
零新数据（`gachaSpCards` 现成 14 张）。定价 20：全游戏 90 钻 = 4 次，每通一章攒得出一次。

☠️ **一个躲开的坑**：抽卡池 SP 卡的 `rarity` 字段是 **'SSR'**，而 `pullCards` 里
`card.rarity === 'SSR' → newPity = 0` 会把**金币池保底清零** —— 钻石抽一次等于把玩家攒的
保底倒扣光。给 `pullCards` 加 `advancePity` 选项，钻石路径传 false。

☠️ **一个当场踩的坑**：第一版 `spendDiamonds` 用朴素函数式 setState，而 `pullCards` 是
「读 stateRef.current → 重建整份 state → 覆盖式 setState」，updater 还没跑就被整份覆盖 ——
**抽完 40 钻纹丝不动**。`spendCoins` 上方有一整段注释记着同一个坑（当年症状"抽卡不消耗金币"），
原样又踩一次。**守卫当时没抓到，是浏览器抓到的** —— 已补 ①-2（扣款函数体内必须出现
`stateRef.current`）并变异验证。

**守卫 `test-diamond-gacha`（27 条，8 个变异先红后绿）**：钻石花得出去且**在 handler 内部**
先查余额（只查"文件里出现过"太松，变异当场抓到）· 扣款用同步 stateRef 模式 ·
**跑真纯函数 300 次断言 100% 出 SP** · 绝不漏出 3 张 campaign_only 的闯关专属 SP ·
不许碰金币保底 · 价格 vs 钻石总量必须落在 2~6 次 · 承诺必须写在按钮上。
为此把 SP 抽卡抽成导出的纯函数 `rollSpCard` —— hook 在 Node 里一调就抛 Invalid hook call，
**包在 hook 里的逻辑守卫测不到**。

📋 `docs/PLAYTEST.md` 同步加了第 4 条取舍：钻石只有通章才给，所以
**「按钮灰着时那句提示会不会让他跑去打闯关」就是这条经济线接没接上的判据**。
