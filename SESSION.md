...
> 更新: 2026-08-25（**事件卡统一到 cqh 已上生产** —— SESSION 里挂最久的一条。
> 病根是「无界内容装进有界卡框」：手牌卡字号还是固定 px，事件卡文案 6~54 字又没有行数封顶。
> iPad 横屏实测「基因突变」溢出 **77px**（文字压到卡外、盖住底部状态栏）；换卡弹窗里事件卡
> 被撑成 251×105 的横条。现在四档视口全部 0 溢出、比例全 0.714。）
>
> 08-23 已在生产：闯关指路 + 按关卡推荐组卡 + 介绍页数字现算。
> 08-22：卡组线两连击（推荐组出非法卡组的真 bug + 新玩家撞墙 + 存量卡组体检）。
> 08-21：首页重构 · 教学四修 + 指向箭头。07-31：4g host 自恢复。
>
> **等齐齐反馈**（已积压 3+ 周，一条没收）：教学五关 / 箭头 / 新首页 / 新卡组界面 /
> **事件卡截断能不能接受** / iPad 横屏观感 / 虎鲸数值。**这些我只能猜，猜不准。**
>
...
## ⚠️ 当前 git / 生产状态
- **HEAD = origin/main = 生产 = `e1a4b6d`**（事件卡统一 cqh），干净树。**测试 75/75 绿**，lint 干净。
- ✅ **生产 = HEAD（2026-08-25 部署 + 字节 + 功能级 + 生产 URL 端到端）**：线上 index.html 引的
  `index-BeaoMA1n.js` / `index-CnyVD8xx.css` / framer / react-vendor，加上 `BattleScreen-CV0APqkq.js`
  与**承载 Card.jsx 钩子的 `Card-CeVaTCS2.js`**，md5 全部逐字节一致。
  线上 CSS 里 `[data-cq-card]` 10 处 / `container: bh-card/size` 1 / `line-clamp:3` 2 /
  降级块 `@container bh-card (max-height:118px)` 1 / 战场卡 P1 A 的 `max(12px,15cqh)` 仍在 2；
  Card chunk 里 `eventdesc`/`sptrigger` 各 1。
  生产 URL 走查（1024×768）：手牌事件卡余量 **-14px**（改前 +77 溢出）、换卡弹窗事件卡
  **74×103 比例 0.714**（改前 251×105 比例 2.39）、控制台无报错。
