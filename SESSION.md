...
- ✅ **E5c-0 `bc2fc2a`**：reducer 首刀，Power Bank 迁进 `src/engine/battleReducer.js`（`useReducer` + `battleStateRef=useLatestRef(battleState)` 供异步 AI 回合/latest 读最新）。POWERBANK_SET/ADD/RESTORE 三 action；6 处写点全改 dispatch；`playerPowerBank/enemyPowerBank` 改派生自 reducer。**证明了 reducer+stateRef 模式在异步 AI 回合成立**（preview 冒烟：回合末 10 能量流入 PB→AI 读 latest.enemyPowerBank→玩家打破 PB 2→12，0 error）。
...
- **E5c-1 = discard**（15 处、纯 append，`canPlayWithMarkers` 读它）→ **E5c-2 = energy**（20 处，+SPEND_ENERGY）→ **E5c-3 = leaderHp**（24 处，注意 `*InitLeaderHpRef` SP 阈值不动、Phase-B useEffect 依赖要改读 reducer）→ **E5c-4 = turn/phase/winner**（`turnRef` 退役）→ **E5c-5 = field**（62 处 + APPLY_COMBAT 原子结算吃掉 applyCombatOutcome 两 setter，风险最高：引用相等/重渲染/动画抖动）→ **E5c-6 收尾**（删净退役 useLatestRef + 更新 grep 锚点 + 新增 `test-battle-reducer.mjs` 纯函数单测 + ARCHITECTURE.md）。
