...
- ✅ **E5c-3 `49dcd67`**：主人 HP 迁进 reducer（最缠一组）。LEADER_DAMAGE/HEAL/SET 三 delta action（累加保多事件链式一致，reducer 纯不碰胜负/阶段）；胜负判定抽到调用端读 battleStateRef 本地算；boss/关卡 setter 垫片；SP 阈值 init ref 不动。preview：AI 突进直攻主人 30000→27000，0 error。
...
- ~~E5c-1 = discard~~ ✅ `c45a4d9` → ~~E5c-2 = energy~~ ✅ `b7d19a8` → ~~E5c-3 = leaderHp~~ ✅ `49dcd67` → **E5c-4 = turn/phase/winner**（turn 3处 + phase ~15处 + winner 少；`turnRef` 退役。⚠️ phase 被多处 useEffect deps 依赖 + BattleScreen 大量读 `battle.phase`、`over`=胜负态、`animating` 由 setAnimating/restorePhase 控——迁前先 grep 所有 `phase` 读点+deps）← **下一刀** → **E5c-5 = field**（62 处 + APPLY_COMBAT 原子结算吃掉 applyCombatOutcome 两 setter，风险最高：引用相等/重渲染/动画抖动）→ **E5c-6 收尾**（删净退役 useLatestRef + 更新 grep 锚点 + 新增 `test-battle-reducer.mjs` 纯函数单测 + ARCHITECTURE.md）。
