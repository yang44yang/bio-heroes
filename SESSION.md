...
> 更新: 2026-08-28（**开发文档对账已上生产**：CLAUDE.md 写 `MAX_FIELD_SLOTS=5` 而代码是 6、
> 卡数 104 vs 124、抽卡概率 85/12/3 vs 68/25/5/2 等六处漂移全部修正 + `test-docs-truth` 钉死。
> 顺手修掉抽卡图鉴进度条「每次打开先闪一下满格」。）
>
> 08-25 已在生产：事件卡统一 cqh（文字不再压到卡外）。08-23：闯关指路 + 按关卡推荐组卡。
> 08-22：卡组线两连击。08-21：首页重构 · 教学四修 + 指向箭头。07-31：4g host 自恢复。
>
> 🔴 **当前唯一瓶颈：齐齐的反馈（积压 3+ 周，一条没收）。**
> 六层界面（教学/首页/卡组/闯关/卡面/文档）全部由我走查驱动改完，守卫防得住回归、
> **防不住方向错**。观察清单见 `docs/PLAYTEST.md`（本次新增）。
>
...
## ⚠️ 当前 git / 生产状态
- **HEAD = origin/main = 生产 = `93ca430`**（开发文档对账 + 守卫），干净树。**测试 76/76 绿**，lint 干净。
- ✅ **生产 = HEAD（2026-08-28 部署 + 字节 + 生产 URL 端到端）**：`index-BI_6k4ih.js` /
  `index-CHhJmTnG.css` / framer / react-vendor，md5 全部逐字节一致。
  生产 URL 走查：抽卡的图鉴进度条内联样式 `width: 12.7389%`（= 20/157 真值，**挂载即真值**）——
  改前是 `width: 329.657px`（从满格往回缩的半途）。控制台无报错。
