...
- ✅ **E5c-4 `37fca8c`**：turn/phase/winner 迁进 reducer。TURN_SET/PHASE_SET/WINNER_SET/GAME_OVER（胜负原子）；三态派生自 reducer、turnRef 退役。外部 battle.phase/turn/winner 按值比较（AI 触发不变）。preview：完整回合机循环 mulligan→main→battle→enemyTurn→AI→R2，0 error。
...
- ~~E5c-1 = discard~~ ✅ `c45a4d9` → ~~E5c-2 = energy~~ ✅ `b7d19a8` → ~~E5c-3 = leaderHp~~ ✅ `49dcd67` → ~~E5c-4 = turn/phase/winner~~ ✅ `37fca8c` → **E5c-5 = field（最险最后一刀）** ← **下一刀** → **E5c-6 收尾**（删净退役 useLatestRef + 更新 grep 锚点 + 新增 `test-battle-reducer.mjs` 纯函数单测 + ARCHITECTURE.md）。
