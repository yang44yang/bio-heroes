...
> 更新: 2026-07-25（**教学守护可见性修复已上生产** —— 齐齐反馈「第三关守护没看出来」：
> 教学迷你卡不显示 skills，守卫兵和普通兵长得一样。现补 🛡️标识 + 被挡的卡变灰）。
> 同日已在生产：教学 4 处卡死修复、iPad 横屏 P1 A（卡锁 5:7 + cqh）、横屏 P0、guest 自选 SP、三技能修复、能量公式。
> 待收反馈：教学能否顺畅打通（含守护看不看得懂）+ P1 A 观感（尤其竖屏变化）+ 虎鲸数值。
>
...
## ⚠️ 当前 git / 生产状态
- **HEAD = origin/main = 生产 = `03f453f`**，干净树。测试 **66/66 绿**，lint 干净。
...
- ☝️ **功能级回验（比对哈希更进一步，值得沿用）**：光对 entry 的 md5 只证明"构建一致"，
  ...
  既比 md5 又数关键字。例：`PvpLobby-DoSAZdQc.js` 线上 `spChoose` 6 处、md5 一致。
  ⚠️ **Tailwind 类名要去 CSS 里找,不在 JS 里**（`03f453f` 验证时 `opacity-40`/`border-cyan-400`
  在 entry 里是 0 处，一度像是部署丢了 —— 实为编译进了 `index-*.css`。判据：**本地 entry 里同样是 0**
  就说明不是部署问题）。另建议加**反向哨兵**：确认被替换掉的旧写法计数为 0（本次 `type==='guard'` → 0 处）。
...
## 已知问题（未修）
- ☠️ **教学引擎的复发模式**（4 处已修 `7ba8cb7`，但同款还会再犯）：`TutorialScreen` 的
  `handlePlayCard`/`handleAttack` 早期**无条件 `advanceStep()`** —— 凡是「要做多次才算达成」的
  步骤（`play_all` 铺满全场 / `clear_field` 清光敌方）都会**做第一次就跳步**，剩下的做不完 → 卡死。
  已改为由既有的「手牌空」/「敌方场空」useEffect 推进。**加新的多次型 waitFor 时必须同样处理**。
  另：`play_card` 步没有结束回合按钮 → 无牌可出必须自动跳（否则卡死，L2/L3 都中过）。
  ⚠️ 教学**没有自动化测试**（脚本+React 状态，Node 测不了）—— 改教学只能 preview 手动走查五关。
- ☠️ **教学迷你卡不走 `Card.jsx`**（`TutorialScreen` 内联渲染，只画 名字/⚔️/❤️/阵营）：
  ...
  等于没教」（齐齐第三关反馈：守卫兵和普通兵长得一模一样，提示却说"有一张守护卡"）。
  判定一律用 `utils/guardSkill` 等**主战场真相源**，别在教学里另写一套（差点开了第 4 处硬编码分叉）。
