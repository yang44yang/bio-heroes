...
> 更新: 2026-08-23（**闯关这条路的三处「说谎」已上生产**：① 关卡弹窗指向已被合并掉的
> 「🃏 卡组」按钮（首页重构留下的）→ 改指「⚡ 一键组卡」；② 一键组卡不看关卡的
> `playerConfig.recommendedFactions`，第三章配不上 → 改为按关卡推荐组、凑不满再兜底；
> ③ 开场介绍页写「60张卡牌/17个关卡」，实际 157 张 / 29 关 → 改为从真相源现算。）
>
> 08-22 已在生产：**卡组线两连击**（一键推荐产出非法卡组的真 bug + 新玩家撞墙 + 存量卡组体检）。
> 08-21：**首页重构**（10 按钮 → 6 + ⚙️ 更多浮层）· **教学四修 + 指向箭头**。07-31：**4g host 自恢复**。
>
> **等齐齐反馈**（已等三周）：教学五关 / 箭头 / 新首页 / 新卡组界面（一键组卡·一键修正）
> / iPad 横屏观感 / 虎鲸数值。**再改下去就是在猜了。**
>
...
## ⚠️ 当前 git / 生产状态
- **HEAD = origin/main = 生产 = `a6cfa2a`**（闯关指路 + 按关卡推荐组卡），干净树。
  **测试 74/74 绿**，lint 干净，中继冒烟 10 条通过（⚠️ 冒烟必须在**没有本地中继占着 3002** 时跑）。
- ✅ **生产 = HEAD（2026-08-23 部署 + 字节 + 功能级 + 生产 URL 端到端）**：线上 index.html 引的
  `index-BDD1LWEM.js` / `index-BK4h_asi.css` / `framer-*` / `react-vendor-*`，加上承载功能的
  lazy chunk `DeckBuilder-A81rPTWn.js`，md5 全部与本地逐字节一致。
  线上 entry：新指路文案 1 处、`{n}张卡牌`/`{n}个关卡` 占位符各 1 处、`recommendedFactions` 26 处；
  **反向哨兵全 0**：`去「🃏 卡组」`、`60张卡牌`、`17个关卡`。
  生产 URL 走查：开场页显示「157张卡牌」「29个关卡」；ch3-2「深海猎场」（推荐 🌱⚗️）一键组卡
  组出的 9 种卡**全是自然系+科技系**、25 张 0 超限、正确进入该关（22,000 HP）；控制台无报错。
