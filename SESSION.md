...
- ✅ **E5c-1 `c45a4d9`**：弃牌堆迁进 reducer。DISCARD_ADD/SET/REMOVE_UID 三 action；~11 读点走 `battleStateRef.current[side].discard`、~10 写点走 dispatch；`playerDiscard/enemyDiscard` 派生自 reducer。3 个 grep 锚点测试同步更新。preview：死卡进弃牌堆「弃牌(1)」+AI 读 latest.enemyDiscard，0 error。
...
- ~~E5c-1 = discard~~ ✅ `c45a4d9` → **E5c-2 = energy**（20 处，+SPEND_ENERGY）← **下一刀** → **E5c-3 = leaderHp**（24 处，注意 `*InitLeaderHpRef` SP 阈值不动、Phase-B useEffect 依赖要改读 reducer）→ **E5c-4 = turn/phase/winner**（`turnRef` 退役）→ **E5c-5 = field**（62 处 + APPLY_COMBAT 原子结算吃掉 applyCombatOutcome 两 setter，风险最高：引用相等/重渲染/动画抖动）→ **E5c-6 收尾**（删净退役 useLatestRef + 更新 grep 锚点 + 新增 `test-battle-reducer.mjs` 纯函数单测 + ARCHITECTURE.md）。
