...
> 更新: 2026-07-24（**iPad 横屏溢出 P0 已上生产** —— 补上 501–780px 紧凑档，卡片内容不再画到卡外；
> 竖屏零回归。更早同日：guest 自选 SP 上线，齐齐当 guest 能自己挑觉醒大招了）。
...
## ⚠️ 当前 git / 生产状态
- **HEAD = origin/main = 生产 = `bbe8a85`**，干净树。测试 **65/65 绿**，lint 干净。
...
### 3. 📱 iPad 横屏 P1（结构性适配）—— P0 已上生产
✅ **P0 已修并部署（`bbe8a85`）**：补上 501–780px 紧凑档，只压「家具」把高度还给战场行，
**没碰卡片布局模型**。实测溢出 +44→−2、竖屏逐项未变；守卫 `test-responsive-tiers`
（最要紧那条：**竖屏 1024 必须落在档位外**）。**但根因没治**：见「已知问题」的长宽比漂移。
- P1 待办：卡片长宽比失控（0.48↔1.83）· 手机横屏 844×390 仍溢出 75px · 横屏黑边。
- `max-w-3xl`(768px) 封顶 → 1024px iPad 两侧各 128px 黑边、12.9 寸各约 299px（浪费约 44% 宽度）。
...
- 🟡 **卡片长宽比不受控**（P0 已止血、根因未治）：卡片没有 `aspect-ratio`，高度是 flex 剩余空间的
  副产品，实测在 **0.48（竖屏，细高）↔ 1.83（横屏矮视口，横条）** 之间漂移。iPad 横屏溢出已由
  P0（`bbe8a85`）修掉，但**手机横屏 844×390 仍溢出 75px**（槽位仅 31px，光压家具救不了）→ 见「下一步」第 3 条。
