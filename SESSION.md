...
- ✅ **E5c-2 `b7d19a8`**：能量迁进 reducer。ENERGY_SET/ADD（cap 可选）/SPEND 三 action；14 写点走 dispatch、4 读点走 battleStateRef；`playerEnergy/enemyEnergy` 派生自 reducer（挪到 battleState 后避 TDZ）。preview：起始⚡10→流入PB+归0→回合 gain⚡2，0 error。
...
- ~~E5c-1 = discard~~ ✅ `c45a4d9` → ~~E5c-2 = energy~~ ✅ `b7d19a8` → **E5c-3 = leaderHp**（24 处，注意 `*InitLeaderHpRef` SP 阈值 ref **不动**、Phase-B 触发 useEffect 依赖 `playerLeaderHp/enemyLeaderHp` 要改读 reducer 派生值）← **下一刀** → **E5c-4 = turn/phase/winner**（`turnRef` 退役）→ **E5c-5 = field**（62 处 + APPLY_COMBAT 原子结算吃掉 applyCombatOutcome 两 setter，风险最高：引用相等/重渲染/动画抖动）→ **E5c-6 收尾**（删净退役 useLatestRef + 更新 grep 锚点 + 新增 `test-battle-reducer.mjs` 纯函数单测 + ARCHITECTURE.md）。
