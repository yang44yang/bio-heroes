...
> 历史更新时间: 2026-07-04 续（**E5c-1 弃牌堆迁进 battleReducer**（`c45a4d9`）：接 E5c-0 第二刀。弃牌堆比 PowerBank 更缠（append+整堆替换+按 uid 移除，~25 处）→ 加 DISCARD_ADD/SET/REMOVE_UID 三 action；`playerDiscard/enemyDiscard` 两 useState+两 useLatestRef → 派生自 reducer，~11 读点走 `battleStateRef.current[side].discard`、~10 写点走 dispatch。语义保真三点：①playToField 的 SET(updatedPile)→ADD([replaced]) 靠 reducer 顺序累加正确 ②sync 回调内 battleStateRef 读渲染快照（与旧 xDiscardRef 一致，onPlay 读 play 前弃牌堆不变）③按引用移除→按 uid（唯一等价）。3 个 grep 锚点测试同步更新（test-hand-events/onDeath-routing/counter-routing，计划 §4⑤ 预判）。全 31/31 绿+build 绿+preview 测试场：AI 出牌读 latest.enemyDiscard+杀卡→死卡进弃牌堆「弃牌(1)」，0 error。**下一步 E5c-2 = energy**（20 处，+SPEND_ENERGY）。）
> 历史更新时间: 2026-07-04（**E5b + E5c-0 reducer 收敛**（2 commit `21a5a6b`/`bc2fc2a`）：**E5b** 停止 useBattle 泄漏原始 `*Ref`——核实 15 个导出 ref 只 8 个真被外部读、7 个死导出，改导出 `latest` 只读 getter 对象（读时取 `.current` 保留「读最新值」、外部拿不到 ref 本体无法写），BattleScreen 4 处 + useAITurn 10 处读取点改走 `battle.latest.xxx`。**E5c-0** reducer 首刀：Power Bank 迁进 `src/engine/battleReducer.js`（`useReducer` + `battleStateRef=useLatestRef(battleState)` 供异步 AI 回合读最新），POWERBANK_SET/ADD/RESTORE 三 action、6 处写点全改 dispatch、`playerPowerBank/enemyPowerBank` 派生自 reducer；**证明 reducer+stateRef 模式在异步 AI 回合成立**。附完整迁移计划 `outputs/e5c-reducer-migration-plan.md`（state shape + action 清单 + 分组切片 E5c-1~6 + 风险）。全 31/31 套绿 + build 绿 + preview 测试场端到端 Power Bank（回合末 10 能量流入 PB→AI 读 latest.enemyPowerBank→玩家打破 PB 2→12，0 error）。**下一步 E5c-1 = discard**（照计划 §5 leaf-first）。⚠️ B/C/D/F+E 全程齐齐仍未真机实测（用户接受、计划建议 E5c-5 后实测一轮）。）
> 历史更新时间: 2026-07-02（**代码体检 + 战斗引擎绞杀式重构 5 连**（5 个新 commit `426547b`→`e7b09f3`）：①**体检+地基**：3 子代理审(引擎正确性/架构/数据一致性)→ `outputs/code-health-report-2026-07-02.md` + `ARCHITECTURE.md`；删死依赖 gsap、加 `npm test` 统一入口(`scripts/run-tests.mjs`)+CI。②**剥 resolveCardCombat**(`src/engine/combat.js`)：attack/aiAttack 伤害/护盾/阵营结算抽成 React-free 纯函数、玩家=AI 单一真相源，`test-combat-resolve` 首个真驱动战斗的单测。③**【B】修 P0**：onAttack/onHit 战斗修饰符打卡静默失效根因(旧写 `ctx.<x>` 被 triggerSkills 拷贝丢弃 + 结算不读)→改「返回事件带 `mods`」(生产端9处)+`aggregateCombatMods` 折叠+resolveCardCombat 消费(damageMultiplier/ignoreShield/dodged/damageReduction)，十余张卡招牌打卡首次生效，直攻主人 RUSH_BOOST→flat×2 不动不双算。④**【C】无视守护**：守护是攻击前的门→`guardSkill.attackerBypassesGuard` 纯谓词单一真相源，useBattle 攻击门+BattleScreen 玩家/AI 选靶共用(精准切除无条件/抗原锁定仅标记)。⑤**【D】3 事件流 bug**：MRSA 反弹镜像 onHitCounter 修路由(`_side:'attacker'`+`ctx.attackerField`)、长老记忆 `_reviveToHand`/信息素召集 `_removeFromHand` 补消费端(`useBattle.applyHandEvents`+BattleScreen 注入 per-side addToHand/playCard)。每刀 test+build+preview 冒烟绿，29 套测试。⚠️**改玩法**：B/C/D 让一批卡效果首次正确生效、齐齐手感会变，务必实测。⚠️**坑**：CI 文件因 git 凭据缺 `workflow` scope 推不上、留磁盘待手动补(`gh auth refresh -s workflow`)；弃牌堆有 useBattle(权威)+useHand(死写)两套。**下一步 F**：描述≠实现批(鲸鲨回血/注射劫持·骨髓造血 `i<3`/PCR 标记/物种大爆发抽2/CRISPR 半互换)。）
...
- **方法（workflow + 内置确定性查重 gate）**：HANDOFF 点名"语义查重"是头号坑。设计成「生成 → 字符二元组 Jaccard 判重 → 撞车自动重改」的 in-script 循环，**判重函数与 test-quiz-system 完全一致**（撞通用题/卡题/本轮 ≥0.5 即重改），从源头挡"同知识点换说法"的重复。数据集内嵌进 workflow 脚本（规避 sandbox 无 fs/args 过大）。
  - **v1**（19 agent 批量 + 3 轮修）：104 道过、43 残留（**41 是"答案最长露馅"格式问题**，非查重）。apply 101 道（i=54/85 origQ 源码重复、i=185 幻觉 index 跳过）。
  - **v2**（重提 46 残留）：强化"正确答案写最短、干扰项写长" + 5 轮上限，**2 轮 46/46 全过**。apply 用 `(origQ+cardId)` 双锚点消歧。
- **质量**：147 道改写 **0 露馅**（答案非最长选项）、**0 撞车**（test-quiz-system 绿）；**全量人工科学 QA 通过**——逐题核对科学事实 + 紧扣卡牌技能（噬菌体钥匙配锁 / 新冠刺突 ACE2 / 大肠杆菌 20 分钟翻倍 / 狂犬沿神经爬 / 麻醉 28 秒截肢 / 李斯特消毒死亡率近半→<1% / 伦琴 X 未知数…），7 岁可懂、无硬错。
- **清理 + 守卫**：28 道残留 legacy 标记（本就是机制/推理题）去掉 → **legacy 概念彻底清零**；test-quiz-system 加决策7段（无 legacy 残留 + 无 legacy+memorization + 卡题机制/推理为主），1476→1479 断言。
- **结果**：卡题 type 记忆 266→**119**（剩的是非-legacy 既有卡题，不在决策7 范围）、机制 259 / 推理 185。745 题不变、全 26 套绿 + build 绿。
- **⚠️ 教训**：① workflow `failed` 池只返回 {i,why} 会丢改写内容（v1 踩、v2 改带 content 返回）。② origQ 可能在源码重复（同题挂两卡）→ apply 必须 (origQ+cardId) 双锚点。③ 露馅靠 agent 自觉不可靠，要把"正确答案最短"写死进 prompt + JS gate 把关。
- **批 0 收官**：决策 1/2/3/6/7 全完成；只剩决策4 dex 收集追踪器（面向产品、留齐齐在场一起做）。
...
- ✅ **E5b `21a5a6b`**：停止泄漏原始 `*Ref`。核实 15 个导出 ref 只有 8 个真被外部读，7 个是死导出。新增 `latest` 只读 getter 对象（读时取 `.current` 保留「读最新值」、外部拿不到 ref 本体）；BattleScreen 4 处 + useAITurn 10 处读取点改走 `battle.latest.xxx`。
- ✅ **E5c-0 `bc2fc2a`**：reducer 首刀，Power Bank 迁进 `src/engine/battleReducer.js`（`useReducer` + `battleStateRef=useLatestRef(battleState)` 供异步 AI 回合/latest 读最新）。POWERBANK_SET/ADD/RESTORE 三 action；6 处写点全改 dispatch；`playerPowerBank/enemyPowerBank` 改派生自 reducer。**证明了 reducer+stateRef 模式在异步 AI 回合成立**。
- ✅ **E5c-1 `c45a4d9`**：弃牌堆迁进 reducer。DISCARD_ADD/SET/REMOVE_UID 三 action；~11 读点走 `battleStateRef.current[side].discard`、~10 写点走 dispatch。preview：死卡进弃牌堆「弃牌(1)」，0 error。
- ✅ **E5c-2 `b7d19a8`**：能量迁进 reducer。ENERGY_SET/ADD（cap 可选）/SPEND 三 action；14 写点走 dispatch、4 读点走 battleStateRef。preview：起始⚡10→流入PB+归0→回合 gain⚡2，0 error。
- ✅ **E5c-3 `49dcd67`**：主人 HP 迁进 reducer（最缠一组）。LEADER_DAMAGE/HEAL/SET 三 delta action（累加保多事件链式一致）；胜负判定抽到调用端读 battleStateRef 本地算；boss/关卡 setter 垫片；SP 阈值 init ref 不动。preview：AI 突进直攻主人 30000→27000，0 error。
- ✅ **E5c-4 `37fca8c`**：turn/phase/winner 迁进 reducer。TURN_SET/PHASE_SET/WINNER_SET/GAME_OVER（胜负原子）；三态派生自 reducer、turnRef 退役。preview：完整回合机循环，0 error。
- ✅ **E5c-5 `3153cef`**：**战场 field 迁进 reducer（收官刀，6/6 组完成）**。FIELD_UPDATE + 透传垫片 + 45 读点走 battleStateRef + 4 类闭包读回/副作用确定性重写（详见顶部）。preview：卡打卡互杀 defKilled+atkKilled 双正确、技能召唤累加正确，0 error。
- ✅ 🧪 测试场 `86ba2bc` + 挂状态增强 `9913a0a`。
**⚠️ 仍未真机实测**：B/C/D/F 改数值 + E1-E5c-0 动热路径，齐齐还没玩过。用户已知并接受「平衡=数值、以后好改」的风险、明确选先推进 E5（别再劝实测）。**但计划 §7 建议：E5c-5（field，最大最险刀）落地后、推进 E5c-6 前请齐齐 iPad 实测一轮。**
**下一步分刀（leaf-first，安全→高杠杆；每刀 test+build+preview 冒烟——preview 用测试场跑 attack+AI回合+死亡+PB）**：照 `outputs/e5c-reducer-migration-plan.md` §5 顺序：
- ~~E5c-1 discard~~ ✅ → ~~E5c-2 energy~~ ✅ → ~~E5c-3 leaderHp~~ ✅ → ~~E5c-4 turn/phase/winner~~ ✅ → ~~E5c-5 field~~ ✅ → ~~E5c-6 收尾~~ ✅（`test-battle-reducer.mjs` 35 断言纯函数单测 + ARCHITECTURE.md 状态模型/测试段更新 + reducer 头注刷新）。**🎉 E5 架构重构全系列（E1→E5c-6）完成，battleReducer 6/6 组迁移收官。**
- ~~E5c-5 对抗审查跟进~~ ✅ `fb5980d`：跑对抗审查 workflow（4 lens+3 verify）揪出 2 confirmed（stageRules 深海 `affected`、bossMechanics 蓝鲸 `events` 两处 updater 闭包读回→banner/浮字丢失）+ 1 正确 refuted。两处硬化（从 field 快照同步算）；finding①（combat defKilled）深挖判**非回归**（老同步读回在有前置事件时同样 deferred）。⚠️ deep_sea/whale 属 ch3 战役关，测试场不覆盖 → **留齐齐战役实测眼验 banner/浮字**。
- **E5 之后可选方向**（都低优先、非阻塞）：① SP 卡组 `playerSpDeck/enemySpDeck` 仍在 useState（边界状态，可后续迁 reducer）；② `attack/aiAttack`、`playToField/aiPlayToField` 两份近重复仍未合并（E3 判定全量合并不值得，但 field 迁 reducer 后 side 差异已收窄，可重新评估）；③ **最该做的其实是让齐齐真机实测**——B/C/D/F + E1~E5c-6 动了大量热路径+数值，一次都还没真机玩过（用户明确接受此风险先推进重构，但重构已收官，实测优先级应回升）。
  - **E5c-5 = field（62 处，最高风险）**：`playerField/enemyField` + APPLY_COMBAT 原子结算（吃掉 applyCombatOutcome 的 defSetter/atkSetter 两个 setState 块）。⚠️ 难点：① field setter（setPlayerField/setEnemyField）被**当参数大量传入** applySkillEvents(friendlySetter,enemySetter)/executeEventEffect/boss·stage 机制/useAITurn → 要么保留 field setter **垫片**（跑 updater→dispatch FIELD_SET，注意同 tick 多次 field 更新的累加，类似 leaderHp 溢出循环的坑）要么全量改 dispatch。② 引用相等：reducer 只 spread 被改的一侧、没改的 field 引用不变，否则动画抖动/无谓重渲染。③ makeFieldCard/护盾结算/死亡扫场 useEffect 依赖 [playerField,enemyField] 要保持触发。**动手前先 grep 所有 setPlayerField/setEnemyField 传参点 + field 读点**，量最大，建议单独一个窗口专注做。
- **套路（E5c-0 已验证，重复即可）**：某组 useState → reducer 子树；`set*` → `dispatch(ACTION)`；`xRef.current` 读 → `battleStateRef.current[side].xxx`；派生 const 供 return 导出；`latest` getter 改读 `battleStateRef`。
- ⚠️ **坑**：① 累加型写（如能量流入）用 reducer 内累加 action（等价 `set(prev=>...)`）保批处理正确，别用「读 ref 算好再 SET」。② reducer 必须只 spread 被改的一侧、没改子树引用不变（防重渲染）。③ grep 锚点测试（test-counter-routing ⑥⑦ / test-phase-b）会随迁移失配，同步改但先确认语义等价。④ E5a 的 `pendingSpSummonRef`（init≠同步值）类坑：ref 迁移手工核对，别信脚本一把过。
