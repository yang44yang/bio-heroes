...
//
// buildFactionDeck 是 DeckBuilder.generateRecommendedDeck 的单阵营·守规版：按阵营筛生物卡 →
// 按 costTargets 铺曲线 → 兜底填满 DECK_SIZE，全程**严格不超 MAX_SAME_CARD**（原 recommended
// 版有「候选不随 main 增长重算 → 可能同名超 3」的隐患，这里逐张 countOf 校验，保证合法）。
...
// 各 cost 段的目标张数（sum=23，其余由兜底填到 25）。照抄 DeckBuilder 的 costTargets 曲线感。
...
function buildFactionDeck(faction) {
  ...
  for (const [cost, target] of Object.entries(COST_TARGETS)) {
    tryFill(pool.filter((c) => c.cost === Number(cost)), target)
  }
  // 2) 兜底填满（任意 cost）
  tryFill(pool, DECK_SIZE)
