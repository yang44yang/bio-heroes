...
├── public/
├── CLAUDE.md
├── CHANGELOG.md               # 历史阶段完成记录（Sprint 1-33 + 决策/Phase + 引擎重构，倒序）
├── SESSION.md                 # 当前会话状态（在根目录！精简：只留活的交接，见「Session 交接规则」）
├── .claude/
│   └── rules/                 # 详细规则（按需加载）
├── package.json
└── vite.config.js
...
### 知识点三标签系统
每个知识点同时标注：
- **Bio Heroes KP_ID**：游戏内主键
- **NGSS 编码**：美国课标对应（如 MS-LS1-1）
- **中国课标**：概念+年级（如"概念四，3-4年级"）

详见 `bio-heroes-knowledge-map.md`（待创建）

### scienceCard 写作原则
- 不只讲"有趣的结论"，要讲"为什么"（原理）
- 技能效果要在 scienceCard 中得到解释
- 7 岁能读懂的语言，但不牺牲科学准确性
...
- **结束会话**（触发词「更新状态」）：更新 `SESSION.md`，按模板记录 最近完成 / 进行中 / 已知问题 / 下次启动时优先 / 关键文件。
  - ⚠️ **必须保持精简**（2026-07-05 精简后的纪律，别再让它膨胀）：SESSION.md 只留「**活的交接**」——当前模式 + 最近 1-2 个大阶段 + 未解决问题 + 活的优先级 + 文件地图，全文控制在 **~100 行内**。
  - **替换、不堆积**：更新时**改写/删旧条目**，绝不在文件顶部累加「历史更新时间」行（那是历史上把它撑到 1000+ 行的最大膨胀源）。完成的旧阶段**浓缩几条归档进 `CHANGELOG.md`**，逐 commit 细节靠 git，不留在 SESSION.md。
  - 划掉的 `~~已修/已完成~~` 项直接删，不留划线噪音。
- **开始会话**（触发词「读取状态」/「继续上次工作」）：先读 `SESSION.md` 了解上次进度
- 历史阶段记录见 `CHANGELOG.md`（Sprint 1-33 + 06 决策/Phase + 07 引擎重构·真机 bug-fix，倒序）
